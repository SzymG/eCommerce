...
/* @var $searchModel common\models\ars\InformationPage */
...
if(empty($withoutDecorations)) {
	if(!empty($searchModel)) {
		$this->title = Yii::t('app', 'Information pages');
...
?>
<div class="information-page-index">

	<?php if(empty($withoutDecorations)) { ?>
    	<h1><?= Html::encode($this->title) ?></h1>
	<?php } else { ?>
    	<h2><?= Yii::t('app', 'Information pages') ?></h2>
	<?php } ?>
    ...
    <p>
    	<?php if(empty($withoutCreating)) { ?>
			<?= Html::a(Yii::t('app', 'Create information page'), ['create'], ['class' => 'btn btn-success']) ?>
		<?php } ?>
		<?php if(!empty($searchModel)) {
