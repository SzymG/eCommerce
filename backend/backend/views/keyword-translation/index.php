...
/* @var $searchModel common\models\ars\KeywordTranslationSearch */
...
if(empty($withoutDecorations)) {
	if(!empty($searchModel)) {
		$this->title = Yii::t('app', 'keywordTranslations');
...
?>
<div class="keyword-translation-index">

	<?php if(empty($withoutDecorations)) { ?>
    	<h1><?= Html::encode($this->title) ?></h1>
	<?php } else { ?>
    	<h2><?= Yii::t('app', 'keywordTranslations') ?></h2>
	<?php } ?>
    ...
    <p>
    	<?php if(empty($withoutCreating)) {
			$url = ['keyword-translation/create'];
            if(isset($keywordId)) {
                $url += ['id' => $keywordId];
            }
			echo Html::a(Yii::t('app', 'createKeywordTranslation'), $url, ['class' => 'btn btn-success']);
		} ?>
		<?php if(!empty($searchModel)) {
    	...
    	<?= GridView::widget([
				...
				'columns' => [
            			...
            			[
								...
								'controller' => 'keyword-translation',
            			],
        		],
    	]); ?>
	<?php Pjax::end(); ?>
